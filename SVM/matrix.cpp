...
Matrix::Matrix(double **rhs, int r, int c)
{ // shallow copy
	rows = r;
	cols = c;
	m = rhs;
...
Matrix::Matrix(vector<vector<double>> &rhs)
{ // 2d vec -> matrix
	rows = rhs.size();
	cols = rhs[0].size();
	m = new double *[rows];
...
double *Matrix::return_col(int c) const
{ // returns cth col
	assert(c < cols);
...
double *Matrix::return_row(int c) const
{ // returns cth row
	assert(c < rows);
...
{ // print matrix with 2 floating point precision
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
...
Matrix Matrix::operator+(double rhs) const
{ // subtract a constant from each element of matrix
	Matrix temp(*this);
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
...
Matrix Matrix::operator/(double rhs) const
{ // divide each element of matrix by a constant
	Matrix temp(*this);
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
...
Matrix Matrix::T(bool self)
{ // returns transpose of matrix, if self is true it updates the object
...
{ // returns max_val and argmax of double dynamic array
	int index = -1;
	for (int i = 0; i < n; i++)
	{
		if (max < abs(col[i]))
...
void Matrix::swap_rows(int index_1, int index_2)
{ // swaps index_1th row with index_2th
	if (index_1 == index_2)
	{
		return;
	}
...
void Matrix::swap_cols(int index_1, int index_2)
{ // swaps index_1th row with index_2th
	if (index_1 == index_2)
	{
		return;
...
Matrix Matrix::gaussianElimination(bool self)
{ // performs gaussian elimination, if self, updates the object
	Matrix temp(*this);
...
Matrix Matrix::remove_row_col(int r, int c, bool self)
{ // remove rth row and cth col
...
{ // computes the cofactor matrix of the object
	Matrix res(rows, cols);
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
...
void Matrix::trimMatrix()
{ // removes false elements due to precision
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
...
{ // returns the inverse of the object
...
pair<Matrix, Matrix> Matrix::qr_decomposition()
{
	...
	for (int i = 0; i < cols; i++)
	{
		...
		for (int j = i; j > 0; j--)
		{ // u_i = a_i - (a_i*e_i-1)e_i-1 - (a_i*e_i-2)e_i-2 - ...
...
Matrix Matrix::elewiseSquare(bool self)
{ // computes elementwise square of the matrix
	Matrix res(*this);
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
...
Matrix Matrix::sum(int axis, bool self)
{ // computes the sum of elements w.r.t. given axis, if self updates the object
	Matrix temp;
	if (axis == 0)
	{
		temp = Matrix(rows, 1);
...
Matrix Matrix::clipRows(int start, int end, bool self)
{ // returns the rows between start, end M[start:end]
...
Matrix Matrix::clipCols(int start, int end, bool self)
{ // returns the columns between start, end M[start:end]
...
void jacobi(double alpha, double beta, double gamma, Matrix &G, double &t)
{ // returns jacobi transformation matrix
	double c = 1, s = 0;
	t = 0;
	G = Matrix(2, 2);
	if (beta != 0)
...
void Matrix::assignCol(Matrix mat, int col)
{ // assigns mat to colth column of the object
	if (mat.getCols() != 1)
	{
...
void Matrix::assignRow(Matrix mat, int row)
{ // assigns mat to rowth row of the object
	if (mat.getRows() != 1)
	{
...
{ // utility function to compare pairs
	return p1.second > p2.second;
}

array<Matrix, 3> Matrix::singular_value_decomposition()
{ // returns U,E,VT where U*E*VT = Object
	Matrix copy_A = *this;
...
{ // returns a diagonal matrix whose components are s
...
Matrix eye(int n)
{ // returns an identity matrix with nxn
	Matrix res(n, n);
	for (int i = 0; i < n; i++)
	{
		res[i][i] = 1;
...
Matrix zeros(int n)
{ // returns a nxn matrix with 0s
	return Matrix(n, n);
}

Matrix ones(int n)
{ // returns a nxn matrix with 1s
	return Matrix(n, n, 1);
}
